...
def refresh_spotify_token(bot, nick, spotify):
    ...
    headers = {"Authorization": "Basic " + b64encode(f"{client_id}:{client_secret}".encode("utf-8")).decode("utf-8")}
    payload = {"grant_type": "refresh_token", "refresh_token": spotify.get("refresh_token")}
...
def get_now_playing(bot, nick, spotify):
    ...
    headers = {"Authorization": f"Bearer {spotify.get('access_token')}"}
...
def spotify_authenticate(bot, trigger):
    ...
    headers = {"Authorization": "Basic " + b64encode(f"{client_id}:{client_secret}".encode("utf-8")).decode("utf-8")}
    payload = {"grant_type": "authorization_code", "code": authcode, "redirect_uri": "https://dsg.fi/bot-callback/"}
